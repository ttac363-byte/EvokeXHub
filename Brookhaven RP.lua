...
-- Variáveis globais pra freeze (reusa selectedPlayerName do dropdown)
getgenv().FreezeEnabled = false
local freezeConnection = nil
local frozenCouch = nil  -- Armazena Couch pra cleanup

-- Função Freeze (usa Couch force sit + lock physics, como no Bring/Kill do hub)
local function FreezePlayer()
    ...
    -- Spawna Couch (como no Bring do hub)
    local args = { [1] = "PickingTools", [2] = "Couch" }
    ReplicatedStorage.RE:FindFirstChild("1Too1l"):InvokeServer(unpack(args))
    ...
    -- Config Couch pra trava (como no Kill Couch: desabilita seats, renomeia Handle)
    frozenCouch = couch
    ...
    local handle = couch:FindFirstChild("Handle")
    if seat1 and seat2 and handle then
        seat1.Disabled = true  -- Trava sit (não sai)
        seat2.Disabled = true
        handle.Name = "Handle "  -- Truque do hub pra lock
    end
    couch.Parent = Character  -- Equipa

    -- Puxa target pra Couch (BodyVelocity follow, como no BringWithCouch do hub)
    local bvPull = Instance.new("BodyVelocity")
    bvPull.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
    bvPull.P = 1250
    bvPull.Velocity = Vector3.new(0, 0, 0)
    bvPull.Name = "FreezePull"
    bvPull.Parent = seat1

    -- Loop pull até sit (como no hub: 35 iterações rápidas)
    spawn(function()
        local iterations = 0
        repeat
            ...
            pos.y = tRoot.Position.Y + (tRoot.Velocity.Y / 2)
            pos.z = tRoot.Position.Z + (tRoot.Velocity.Z / 2)
            seat1.CFrame = CFrame.new(pos) * CFrame.new(-2, 2, 0)  -- Posiciona atrás/alto
            wait(0.05)  -- Rápido pra puxar
        until tHum.Sit == true or iterations > 35 or not getgenv().FreezeEnabled

        bvPull:Destroy()  -- Cleanup pull
    end)

    wait(math.random(0.5, 1))  -- Delay até sit confirmado

    -- Trava pós-sit (desabilita states + BodyVelocity=0, como Anti Sit + fling physics)
    tHum.PlatformStand = true  -- Ragdoll trava (replica server-side)
    tHum:SetStateEnabled(Enum.HumanoidStateType.Running, false)  -- No run
    tHum:SetStateEnabled(Enum.HumanoidStateType.Jumping, false)  -- No jump

    -- BodyVelocity=0 pra lock total (MaxForce huge replica pra todos)
    ...
    -- Loop reforça trava infinita (como RunService no fling do hub)
    if freezeConnection then freezeConnection:Disconnect() end
    freezeConnection = RunService.Heartbeat:Connect(function()
        if getgenv().FreezeEnabled and tRoot and tRoot.Parent then
            lockBV.Velocity = Vector3.new(0, 0, 0)  -- Zera velocidade
    ...
    print("Player " .. selectedPlayerName .. " frozen with Couch lock (visible to all)!")
end
...
local function UnfreezePlayer()
    ...
    if target and target.Character then
        ...
        if tHum then
            ...
            tHum:ChangeState(Enum.HumanoidStateType.GettingUp)  -- Levanta se ragdoll
        end
        if tRoot then
    ...
    -- Cleanup Couch (reset como no hub)
    if frozenCouch then
        frozenCouch.Parent = LocalPlayer.Backpack
        local handleSpace = frozenCouch:FindFirstChild("Handle ")
        if handleSpace then handleSpace.Name = "Handle" end
...
-- UI: Toggle Freeze (integra com o dropdown existente)
Troll:AddToggle({
    Name = "Freeze Player (Couch Lock)",
    Description = "Freezes selected player (stuck, visible to all)",
    Default = false,
    Callback = function(value)
...
-- Button rápido (opcional, pra freeze instantâneo)
Troll:AddButton({
    Name = "Quick Freeze",
    Callback = function()
